...
| 1.2 | Buat laporan baru dengan deskripsi **sengaja singkat** | `/warga/create-report` | - | Laporan tersubmit | `[ ]` | Misal deskripsi: "Jalan rusak" (tanpa detail) |
...
| 2.6 | **Submit minta revisi** | POST `/admin/validation` action=revision | Flash message | "Laporan Perlu Revisi" | `[ ]` | |
| 2.7 | Cek laporan masih ada di antrian? | `/admin/validation` | Daftar | ⚠️ Laporan PERLU_REVISI: apakah masih tampil atau hilang? | `[ ]` | |
...
| 3.3 | Cek catatan admin tampil | - | 📋 Catatan | Teks catatan dari admin tampil | `[ ]` | |
...
| 3.7 | Update patokan lokasi | - | Field patokan | Bisa diubah | `[ ]` | |
| 3.8 | Upload foto baru (opsional) | - | Upload foto | Bisa upload ulang | `[ ]` | |
| 3.9 | **Submit revisi** | POST `/warga/revise-report` | Flash message | "Revisi laporan berhasil dikirim. Admin akan meninjau kembali." | `[ ]` | |
...
| 4.3 | **Approve laporan** | POST `/admin/validation` action=approve | Flash message | "Laporan Divalidasi" | `[ ]` | |
...
| 5.2 | Admin dinas tugaskan petugas | Pilih petugas | DITUGASKAN | `[ ]` |
| 5.3 | Petugas check-in + start tugas | Geofencing | SEDANG_DIKERJAKAN | `[ ]` |
| 5.4 | Petugas upload foto + selesaikan | Before + After | MENUNGGU_KONFIRMASI | `[ ]` |
...
- [ ] Status laporan mengikuti: MENUNGGU → PERLU_REVISI → MENUNGGU → DIVALIDASI → ... → SELESAI
- [ ] Catatan revisi dari admin tampil di halaman warga
